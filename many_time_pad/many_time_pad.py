import math

ciphertexts = [
    ,
    0x234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f,
    0x32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb,
    ,
    0x3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070,
    ,
    ,
    ,
    ,
    0x466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83
]

target_cipher = 0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904


def byteXOR(x, y):
    length = min(len(x), len(y))
    result = bytearray(length)
    for idx in range(length):
        result[idx] = x[idx] ^ y[idx]
    return result


def int2bytes(num):
    return num.to_bytes(math.ceil(num.bit_length() / 8), 'big')


if __name__ == '__main__':
    # the difference between lower case letter and the corresponding upper case letter is 32 (0x20),
    # which is exactly the ascii value of space.
    ciphertexts = [int2bytes(cipher) for cipher in ciphertexts]
    key = bytearray(1024)
    space_locations = [{} for i in range(len(ciphertexts))]
    for cipher_idx_1 in range(len(ciphertexts)):
        for cipher_idx_2 in range(len(ciphertexts)):
            if cipher_idx_1 != cipher_idx_2:
                message_xor_as_bytes = byteXOR(ciphertexts[cipher_idx_1], ciphertexts[cipher_idx_2])
                for byte_idx in range(len(message_xor_as_bytes)):
                    if ord('a') <= message_xor_as_bytes[byte_idx] <= ord('z') or ord('A') <= message_xor_as_bytes[byte_idx] <= ord('Z') or message_xor_as_bytes[byte_idx] == 0:
                        # here, one byte is a letter and the other is space...
                        if byte_idx in space_locations[cipher_idx_1]:
                            space_locations[cipher_idx_1][byte_idx] += 1
                        else:
                            space_locations[cipher_idx_1][byte_idx] = 1

    print(space_locations)
    for cipher_idx in range(len(space_locations)):
        for location, count in space_locations[cipher_idx].items():
            if count > 6:
                key[location] = ciphertexts[cipher_idx][location] ^ ord(' ')

    print('key=', key)
    target_cipher = int2bytes(target_cipher)
    message = byteXOR(key, target_cipher)
    print(message.decode('latin1'))
